var AppProcess = (function () {
    ...
    function eventProcess() {
        ...
        $("#miceMuteUnmute").on("click", async function () {
            try {
                ...
                isAudioMute = !isAudioMute; // Flip the mute state
            } catch (error) {
                console.error("Error toggling audio: ", error);
                alert("An error occurred while toggling the microphone.");
            }
        });

        // Toggle video camera on/off
        $("#VideoCamOnOff").on("click", async function () {
            console.log(video_st,video_states)
            try {
                ...
                console.error("Error toggling video camera: ", error);
                alert("An error occurred while toggling the video camera.");
            }
        });
        ...
        $("#ScreenShareOnOff").on("click", async function () {
            try {
                ...
                alert("An error occurred while toggling screen sharing.");
            }
        });
    }
        ...
        for (var con_id in peers_connection_ids) {
            if (connection_status(peers_connection[con_id])) {
                if (rtp_senders[con_id] && rtp_senders[con_id].track) {
                    await rtp_senders[con_id].replaceTrack(track)
                }
                else {
    ...
    function removeMediaSenders(rtp_senders) {
        for (var con_id in peers_connection_ids) {
            console.log("status",connection_status(peers_connection[con_id]))
    ...
    async function videoProcess(newVideoState) {
        try {
            // Handle stopping video (None state)
            if (newVideoState === video_states.None) {
                ...
                console.log("closing stream",rtp_vid_senders);
                removeVideoStream(rtp_vid_senders); // Stop video streaming
                return;
            }

            // Update button UI for Camera state
            if (newVideoState === video_states.Camera) {
                $("#VideoCamOnOff").html("<span class='material-icons' style='width: 100%;'>videocam</span>");
            }

            // Initialize video stream variable
            let vstream = null;
            ...
            if (newVideoState === video_states.Camera) {
                ...
                vstream.getVideoTracks()[0].onended = () => {
                    videoProcess(video_states.None); // Automatically revert to None state
                };
            }

            // Process and display video stream
            if (vstream && vstream.getVideoTracks().length > 0) {
                ...
                if (videoCamTrack) {
                    ...
                    updateMediaSenders(videoCamTrack, rtp_vid_senders); // Update sender streams
                }
            }
            ...
            console.error("Error in videoProcess:", error);
        }
    }
...
var MyApp = (function () {
    ...
    function event_process_for_signaling_server() {
        ...
        socket.on("showFileMessage", function(data){
            ...
            attachFileAreaForOther.innerHTML += "<div class='left-align' style='display: flex; align-items: center;'><img src = 'public/Assets/images/image.png' style='height: 40px; width: 40px;' class='caller-image circle'><div style='font-weight: 600; margin: 0 5px;'>"+data.username+"</div>:<div><a style='color: #007bff;' href='" + data.filePath + "' download>" + data.fileName + "</a></div></div><br/>";
        })
    ...
    function eventHandling() {
        $("#btnsend").on("click", function () {
            ...
            var div = $("<div>").html("<span class='font-weight-bold mr-3' style='color: black'>" + user_id + "</span>" + ltime + "</br>" + $("#msgBox").val())
            $("#messages").append(div);
            if (message.trim() !== "") {
                socket.emit("sendMessage", message);
                $("#msgBox").val(""); // Clear the input box
            }
        });
    ...
    $(document).on("click", ".share-attach", function (e) {
        ...
        attachFileArea.innerHTML += "<div class='left-align' style='display: flex; align-items: center;'><img src = 'public/Assets/images/image.png' style='height: 40px; width: 40px;' class='caller-image circle'><div style='font-weight: 600; margin: 0 5px;'>"+user_id+"</div>:<div><a style='color: #007bff;' href='" + attachFilePath + "' download>" + attachFileName + "</a></div></div><br/>";
    ...
    $(document).on("click", ".stop-record", function(){
        $(this).removeClass().addClass("stop-record btn-dark text-danger").text("Start Recording");
        mediaRecorder.stop();
    });
